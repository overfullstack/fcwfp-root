...
class RailwayValidation2Test {
  ...
  void plainOldImperativeOrchestration() {
    final var validationResults =
        runAllValidationsFailFastImperative(
            IMMUTABLE_EGG_CARTON,
            EGG_VALIDATION_CHAIN,
            NOTHING_TO_VALIDATE,
            ValidationFailure::withThrowable);
    Assertions.assertEquals(
        EXPECTED_DECLARATIVE_VALIDATION_RESULTS.toJavaList(), validationResults);
  }
  ...
  void failFast1() {
    final var validationResults =
        IMMUTABLE_EGG_CARTON
            ...
            .map(
                failFastStrategy(
                    EGG_VALIDATION_CHAIN, NOTHING_TO_VALIDATE, ValidationFailure::withThrowable))
            .toList();
  ...
  void failFastNonBulk1() {
    final var validationResult =
        failFastStrategy(
                EGG_VALIDATION_CHAIN, NOTHING_TO_VALIDATE, ValidationFailure::withThrowable)
            .apply(IMMUTABLE_EGG_CARTON.get());
  ...
  void noValidations1() {
    final var validationResult =
        failFastStrategy(List.empty(), NOTHING_TO_VALIDATE, ValidationFailure::withThrowable)
            .apply(IMMUTABLE_EGG_CARTON);
    LOGGER.info(validationResult.toString());
  }
  ...
  void noValidations2() {
    final var validationResult =
        failFastStrategy2(List.empty(), NOTHING_TO_VALIDATE, ValidationFailure::withThrowable)
            .apply(IMMUTABLE_EGG_CARTON);
    LOGGER.info(validationResult.toString());
  }
  ...
  void errorAccumulation() {
    final var validationResultsAccumulated =
        IMMUTABLE_EGG_CARTON
            .iterator()
            .map(
  ...
  void parallel1() {
    final var validationResults =
        getStreamBySize(IMMUTABLE_EGG_CARTON)
            .map(
                failFastStrategy(
                    EGG_VALIDATION_CHAIN, NOTHING_TO_VALIDATE, ValidationFailure::withThrowable))
  ...
  void parallel2() {
    ...
    validationResults.forEach(result -> LOGGER.info(result.toString()));
    Assertions.assertEquals(
        EXPECTED_DECLARATIVE_VALIDATION_RESULTS_2.toJavaList(), validationResults);
  }
}
