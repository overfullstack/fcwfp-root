...
public final class Strategies {
    ...
    for (var toBeValidated : validatables) {
      if (toBeValidated == null) {
        ...
        for (var currentValidation : validations) {
          validationResult = fireValidation(currentValidation, toBeValidatedRight, throwableMapper).map(
              ignore -> toBeValidated);
  ...
  /**
   * - No need to comprehend every-time, like the nested for-each loop - No need to Unit-test - Shared Vocab within the
   * team - Universal Vocab across languages
   */
    ...
    return toBeValidated -> {
      if (toBeValidated == null) {
        ...
        final BiFunction<Either<FailureT, ?>, Validator<ValidatableT, FailureT>, Either<FailureT, ?>> applyValidation
            = (validated, currentValidation) -> validated.isRight() ? fireValidation(
            currentValidation, toBeValidatedRight, throwableMapper) : validated;
      ...
      if (toBeValidated == null) {
        ...
        return validations.foldLeft(
            ...
            (failureResults, currentValidation) -> {
              final var result = fireValidation(currentValidation, toBeValidatedRight, throwableMapper);
