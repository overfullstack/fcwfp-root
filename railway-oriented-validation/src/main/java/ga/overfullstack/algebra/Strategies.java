...
/**
 * These Strategies compose multiple validations and return a single function which can be applied
 * on the Validatable.
 *
 * <p>gakshintala created on 3/21/20.
 */
public final class Strategies {

  private Strategies() {}

  public static <FailureT, ValidatableT>
      java.util.List<Either<FailureT, ValidatableT>> runAllValidationsFailFastImperative(
    ...
    for (var toBeValidated : validatables) {
      if (toBeValidated == null) {
        ...
        for (var currentValidation : validations) {
          validationResult =
              fireValidation(currentValidation, toBeValidatedRight, throwableMapper)
                  .map(ignore -> toBeValidated);
  ...
  /**
   * - No need to comprehend every-time, like the nested for-each loop - No need to Unit-test -
   * Shared Vocab within the team - Universal Vocab across languages
   */
  public static <FailureT, ValidatableT> FailFastStrategy<ValidatableT, FailureT> failFastStrategy(
    ...
    return toBeValidated -> {
      if (toBeValidated == null) {
        ...
        Either<FailureT, ValidatableT> toBeValidatedRight = Either.right(toBeValidated);
        final BiFunction<
                Either<FailureT, ?>, Validator<ValidatableT, FailureT>, Either<FailureT, ?>>
            applyValidation =
                (validated, currentValidation) ->
                    validated.isRight()
                        ...
                        : validated;
        return validations
            .foldLeft(toBeValidatedRight, applyValidation)
            .map(ignore -> toBeValidated);
      }
    };
  }
  ...
  public static <FailureT, ValidatableT>
      AccumulationStrategy<ValidatableT, FailureT> accumulationStrategy(
          List<Validator<ValidatableT, FailureT>> validations,
          FailureT invalidValidatable,
          Function1<Throwable, FailureT> throwableMapper) {
    return toBeValidated -> {
  ...
  private <FailureT, ValidatableT>
      AccumulationStrategy<ValidatableT, FailureT> accumulationStrategy2(
          List<Validator<ValidatableT, FailureT>> validations,
          FailureT invalidValidatable,
          Function1<Throwable, FailureT> throwableMapper) {
    return toBeValidated ->
        toBeValidated == null
            ...
            : applyValidations(toBeValidated, validations, throwableMapper)
                .map(
                    result ->
                        result.map(
                            ignore -> toBeValidated)) // replacing wildcard on right state with
                // toBeValidated.
                .toList();
  }
  ...
  private static <FailureT, ValidatableT> Iterator<Either<FailureT, ?>> applyValidations(
    ...
    Either<FailureT, ValidatableT> toBeValidatedRight = Either.right(toBeValidated);
    return validations
        .iterator() // This is vavr iterator, which is lazy by default.
        .map(
            currentValidation ->
                fireValidation(currentValidation, toBeValidatedRight, throwableMapper));
  }
