...
public class DataSet {
  ...
  public static final List<ImmutableEgg> IMMUTABLE_EGG_CARTON =
      io.vavr.collection.List.of( // Using vavr list as `java.util.List` doesn't allow `null` in `List.of()`
          ...
          new ImmutableEgg(-1, new Yolk(Condition.BAD, Color.GOLD)), // Chicken might already be out
          ...
          new ImmutableEgg(0, new Yolk(Condition.BAD, Color.YELLOW)), // Chicken might already be out
  ...
  public static final List<Either<ValidationFailure, ?>> EXPECTED_DECLARATIVE_VALIDATION_RESULTS = List.of(
      ...
      Either.left(ValidationFailure.withErrorMessage(ThrowableMsgs.THROWABLE_NESTED_OPERATION_32)),
