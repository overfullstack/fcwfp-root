...
fun <FailureT, ValidatableT> failFastStrategy(
  ...
  when (validatable) {
    null -> invalidValidatable.left()
      ...
      validations
        .fold(validatable.right() as Either<FailureT, Any?>) {
        ...
        .map {
          validatable
        } // To put back the original validatable in place of `Any?` in right state.
    }
  }
}
...
fun <FailureT, ValidatableT> accumulationStrategyInParallel(
  ...
  when (validatable) {
    null -> listOf(invalidValidatable.left())
      ...
      validations
        .parMap { validation -> fireValidation(validation, validatable, throwableMapper) }
        .map { it.map { validatable } }
    }
  }
}

private fun <FailureT, ValidatableT> fireValidation(
  validation: Validator<ValidatableT, FailureT>,
  validatable: ValidatableT,
  throwableMapper: (Throwable) -> FailureT,
): Either<FailureT, Any?> =
  Either.catch { validation(validatable) }.fold({ throwableMapper(it).left() }, ::identity)
...
suspend fun <FailureT, ValidatableT> runAllValidationsFailFastStrategyImperative(
  ...
  throwableMapper: (Throwable) -> FailureT,
  invalidValidatable: FailureT,
): MutableList<Either<FailureT, ValidatableT?>> {
  var validationResults = mutableListOf<Either<FailureT, ValidatableT?>>()
  for (validatable in validatables) {
    val either: Either<FailureT, ValidatableT?> =
      when (validatable) {
        null -> invalidValidatable.left()
        else -> {
    ...
    validationResults.add(either) // 🚩 Mutation
  }
