 ...
 *
 * Requirements ∙ Partial Failures
 *
 * Problems solved: ∙ Octopus Orchestrator - 😵 dead ∙ Mutation to Transformation ∙
 * Unit-Testability - 👍 ∙ Complexity - Minimum ∙ Chaos to Order
 */
...
val validateChild31X: Validator<Yolk, ValidationFailure> = {
  either<ValidationFailure, Boolean> { throwableNestedRule(it) }
...
val validateParent41X: Validator<Egg, ValidationFailure> = {
  either<ValidationFailure, Boolean> { throwableRule3(it) }
    .filterOrElse(::identity) { ABOUT_TO_HATCH_P_3 }
}
