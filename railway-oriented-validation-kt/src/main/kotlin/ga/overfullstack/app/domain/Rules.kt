...
/**
 * Assume these operations are some side-effects which are not in our control. These throw
 * exceptions for various cases.
 */
...
fun throwableRule2(eggToBeValidated: Egg): Boolean =
...
fun throwableRule3(eggToBeValidated: Egg): Boolean =
...
fun throwableNestedRule(yolkTobeValidated: Yolk?): Boolean =
  when {
    yolkTobeValidated == null -> throw IllegalArgumentException(THROWABLE_NESTED_OPERATION_31)
    yolkTobeValidated.condition === Condition.BAD ->
      throw IllegalArgumentException(THROWABLE_NESTED_OPERATION_32)
