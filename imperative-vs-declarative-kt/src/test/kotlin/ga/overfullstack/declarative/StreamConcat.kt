...
package ga.overfullstack.declarative
...
class StreamConcat {
  ...
  companion object {
    private fun concatLastNames1(team: List<String>): String {
      return team.asSequence().map(::extractLastName).joinToString(DELIMITER)
    }

    /**
     * ∙ Patching up -> Extending ∙ Control Statements -> Expressions ∙ Code that talks ∙ Puzzle
     * pieces fit together.
     */
    fun concatLastNames(team: List<String?>?): String =
      (team?.asSequence() ?: emptySequence()) // HTD-1: Looping
    ...
    fun concatLastNamesInParallel(team: List<String?>?): String =
      (team?.parallelStream() ?: Stream.empty()) // HTD-1
